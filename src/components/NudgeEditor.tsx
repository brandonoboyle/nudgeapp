...
export interface EditorAccent {
  /** CSS variable for caret/cursor/selection, e.g. '--master-gold' */
  accentVar: string;
  /** CSS variable for selection background dim, e.g. '--master-gold-dim' */
  accentDimVar: string;
  /** Placeholder text shown in empty editor */
  placeholder: string;
  /** Badge label shown in toolbar, e.g. 'The Draft' */
  badgeLabel: string;
  /** Whether to show character count in footer */
  showCharCount?: boolean;
}

interface NudgeEditorProps {
  file: NudgeFile;
  accent: EditorAccent;
  styles: Record<string, string>;
}

function buildTheme(accent: EditorAccent) {
  return EditorView.theme({
    ...
    '.cm-content': {
      ...
      caretColor: `var(${accent.accentVar})`,
      padding: '0',
    },
    '.cm-cursor, .cm-dropCursor': {
      borderLeftColor: `var(${accent.accentVar})`,
      borderLeftWidth: '2px',
    },
    ...
    '.cm-selectionBackground': { background: `var(${accent.accentDimVar})` },
    '&.cm-focused .cm-selectionBackground': { background: `var(${accent.accentDimVar})` },
    '.cm-placeholder': { color: 'var(--text-muted)', fontStyle: 'italic' },
  });
}

export default function NudgeEditor({ file, accent, styles }: NudgeEditorProps) {
  ...
  // Stable ref for the theme — only rebuild if accent identity changes
  const themeRef = useRef(buildTheme(accent));
    ...
    const view = new EditorView({
      state: EditorState.create({
        ...
        extensions: [
          ...
          cmPlaceholder(accent.placeholder),
    ...
    <div className={styles.editor}>
      <div className={styles.toolbar}>
        <div className={styles.toolbarGroup}>
          <button className={styles.toolbarBtn} title="Heading 1 (H1)" onClick={() => insertHeading(1)}>H1</button>
          <button className={styles.toolbarBtn} title="Heading 2 (H2)" onClick={() => insertHeading(2)}>H2</button>
          <button className={styles.toolbarBtn} title="Heading 3 (H3)" onClick={() => insertHeading(3)}>H3</button>
        </div>
        <div className={styles.toolbarDivider} />
        <div className={styles.toolbarGroup}>
          <button className={`${styles.toolbarBtn} ${styles.boldBtn}`} title="Bold (⌘B)" onClick={() => wrapSelection('**')}>
            <b>B</b>
          </button>
        ...
        <span className={styles.badge}>{accent.badgeLabel}</span>
      </div>
