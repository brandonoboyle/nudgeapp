...
const nudgeTheme = EditorView.theme({
  ...
  '.cm-content': {
    ...
    caretColor: 'var(--master-gold)',
    padding: '0',
  },
  '.cm-cursor, .cm-dropCursor': {
    borderLeftColor: 'var(--master-gold)',
    borderLeftWidth: '2px',
  },
  ...
  '.cm-selectionBackground': { background: 'var(--master-gold-dim)' },
  '&.cm-focused .cm-selectionBackground': { background: 'var(--master-gold-dim)' },
...
export default function MasterEditor({ file }: MasterEditorProps) {
  ...
  const wordCount = useMemo(
    ...
    // We track content via ref, so we use file.content as a proxy for initial render
  ...
  // Store refs for use in CodeMirror callbacks (avoids stale closures)
  ...
  useEffect(() => {
    ...
    // Only re-mount when file identity changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [file.id]);
    ...
    <div className={styles.masterEditor}>
      <div className={styles.toolbar}>
        <div className={styles.toolbarGroup}>
          <button className={styles.toolbarBtn} title="Heading 1 (H1)" onClick={() => insertHeading(1)}>
            H1
          </button>
          <button className={styles.toolbarBtn} title="Heading 2 (H2)" onClick={() => insertHeading(2)}>
            H2
          </button>
          <button className={styles.toolbarBtn} title="Heading 3 (H3)" onClick={() => insertHeading(3)}>
            H3
          </button>
        </div>
        ...
        <span className={styles.masterBadge}>The Draft</span>
      </div>
